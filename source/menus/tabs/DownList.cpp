...
void DownList::DrawTopOvl() {
	...
	if (!UU::App->Store->Indexes.empty() && UU::App->Store->SelectedIndex <= UU::App->Store->Indexes.size() - 1) {
		...
		if (!Entries.empty()) {
			...
			Gui::DrawString(70, 174 + 15, 0.45f, TEXT_COLOR, Entries[this->SelectedIndex], 310, 0);
			...
			if (Size != "") {
				Gui::DrawString(70, 174 + 30, 0.45f, TEXT_COLOR, "Size: " +  Size, 310, 0);
			}
		}
	}
};
...
void DownList::Draw() {
	...
	Gui::DrawStringCentered(17, 2, 0.6, TEXT_COLOR, "Available Downloads", 273, 0);
	...
	if (!UU::App->Store->Indexes.empty() && UU::App->Store->SelectedIndex <= UU::App->Store->Indexes.size() - 1) {
		...
		if (!Entries.empty()) {
			for (size_t Idx = 0; Idx < DOWNLOAD_ENTRIES && Idx < Entries.size(); Idx++) {
				...
				Gui::DrawStringCentered(46 - 160 + (241 / 2), this->DownPos[Idx].y + 4, 0.45f, TEXT_COLOR, Entries[Idx + this->ScreenIndex], 235, 0);
			...
			Gui::DrawStringCentered(46 - 160 + (241 / 2), this->DownPos[0].y + 4, 0.5f, TEXT_COLOR, "No Downloads available.", 235, 0);
		}
	}
};
