...
void StoreUtils::DrawEntryInfo(const std::shared_ptr<StoreEntry> &entry) {
	if (StoreUtils::store && entry) { // Ensure, store & entry is not a nullptr.
		...
		std::string subtitle = entry->GetAuthor();
		if(entry->GetStars())
...
*/
void StoreUtils::EntryHandle(u32 hDown, touchPosition &touch, bool &showMark, bool &sFetch, int &mode, const std::shared_ptr<StoreEntry> &entry) {
	if (entry) {
