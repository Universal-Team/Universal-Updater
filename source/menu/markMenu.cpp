...
void StoreUtils::DisplayMarkBox(int marks) {
	...
	Gui::Draw_Rect(markBox[0].x, markBox[0].y, markBox[0].w, markBox[0].h, ((marks & favoriteMarks::STAR) ?
		UITheme.MarkSelected() : UITheme.MarkUnselected()));
	...
	Gui::DrawString(markBox[0].x + 15, markBox[0].y + 11, 0.9, UITheme.TextColor(), "★", 0, 0, font);
	Gui::DrawString(markBox[1].x + 15, markBox[1].y + 11, 0.9, UITheme.TextColor(), "♥", 0, 0, font);
	Gui::DrawString(markBox[2].x + 15, markBox[2].y + 11, 0.9, UITheme.TextColor(), "♦", 0, 0, font);
	Gui::DrawString(markBox[3].x + 15, markBox[3].y + 11, 0.9, UITheme.TextColor(), "♣", 0, 0, font);
	Gui::DrawString(markBox[4].x + 15, markBox[4].y + 11, 0.9, UITheme.TextColor(), "♠", 0, 0, font);

	GFX::DrawBox(markBox[5].x, markBox[5].y, markBox[5].w, markBox[5].h, false);
	Gui::DrawString(markBox[5].x + 5, markBox[5].y + 2, 0.6f, UITheme.TextColor(), "★", 0, 0, font);
}
	...
	if (StoreUtils::meta && entry && StoreUtils::store) {
		if (hidKeysDown() & KEY_TOUCH) {
			...
			if (touching(t, markBox[0])) {
				StoreUtils::meta->SetMarks(entry->GetUniStore(), entry->GetTitle(),
					StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()) ^ favoriteMarks::STAR);
				entry->SetMark(StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()));

			/* Heart. */
			} else if (touching(t, markBox[1])) {
				StoreUtils::meta->SetMarks(entry->GetUniStore(), entry->GetTitle(),
					StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()) ^ favoriteMarks::HEART);
				entry->SetMark(StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()));

			/* Diamond. */
			} else if (touching(t, markBox[2])) {
				StoreUtils::meta->SetMarks(entry->GetUniStore(), entry->GetTitle(),
					StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()) ^ favoriteMarks::DIAMOND);
				entry->SetMark(StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()));

			/* Clubs. */
			} else if (touching(t, markBox[3])) {
				StoreUtils::meta->SetMarks(entry->GetUniStore(), entry->GetTitle(),
					StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()) ^ favoriteMarks::CLUBS);
				entry->SetMark(StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()));

			/* Spade. */
			} else if (touching(t, markBox[4])) {
				StoreUtils::meta->SetMarks(entry->GetUniStore(), entry->GetTitle(),
					StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()) ^ favoriteMarks::SPADE);
				entry->SetMark(StoreUtils::meta->GetMarks(entry->GetUniStore(), entry->GetTitle()));
			}
		}
	}
