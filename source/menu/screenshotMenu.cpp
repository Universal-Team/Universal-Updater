...
void StoreUtils::DrawScreenshotMenu(const C2D_Image &img, const int sIndex, const bool sFetch, const int screenshotSize, const std::string &name, const int zoom, const bool canDisplay) {
	...
	if (sFetch) {
		...
		GFX::DrawBottom();
		Gui::DrawStringCentered(0, 2, 0.6f, UITheme.TextColor(), Lang::get("LOADING_SCREENSHOT"), 310);
		return;
	}
	...
	if (!canDisplay) {
		...
		if (screenshotSize > 0) { // if texture is nullptr AND screenshot size is larger than 0.
			Gui::DrawStringCentered(0, 2, 0.6f, UITheme.TextColor(), Lang::get("SCREENSHOT_COULD_NOT_LOAD"), 310);

		} else {
			Gui::DrawStringCentered(0, 2, 0.6f, UITheme.TextColor(), Lang::get("NO_SCREENSHOTS_AVAILABLE"), 310);
		}
	...
	if (!sFetch) { // Only, if not fetch. This avoids a small flicker of the old screenshot on entries without screenshots.
		if (screenshotSize > 0) {
			...
			if (zoom > 0 && img.subtex->height * scale > 240) {
				...
				Gui::DrawStringCentered(0, 220, 0.5f, UITheme.TextColor(), Lang::get("SCREENSHOT_INSTRUCTIONS"), 310, 0, font);

				char screenshots[0x100];
				snprintf(screenshots, sizeof(screenshots), Lang::get("SCREENSHOT").c_str(), sIndex + 1, screenshotSize);
				Gui::DrawStringCentered(0, 2, 0.6f, UITheme.TextColor(), screenshots, 310, 0, font);
				Gui::DrawStringCentered(0, 40, 0.6f, UITheme.TextColor(), name, 310, 0, font);
			}
